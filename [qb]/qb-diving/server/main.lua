...
QBCore.Functions.CreateCallback('qb-diving:server:GetMyBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = exports.oxmysql:executeSync('SELECT * FROM player_boats WHERE citizenid=@citizenid AND boathouse=@boathouse', {['@citizenid'] = Player.PlayerData.citizenid, ['@boathouse'] = dock})
...
QBCore.Functions.CreateCallback('qb-diving:server:GetDepotBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = exports.oxmysql:executeSync('SELECT * FROM player_boats WHERE citizenid=@citizenid AND state=@state', {['@citizenid'] = Player.PlayerData.citizenid, ['@state'] = 0})
    if result[1] ~= nil then
        cb(result)
    else
        cb(nil)
...
AddEventHandler('qb-diving:server:SetBoatState', function(plate, state, boathouse, fuel)
    ...
    if result ~= nil then
        exports.oxmysql:execute('UPDATE player_boats SET state=@state, fuel=@fuel WHERE plate=@plate AND citizenid=@citizenid', {['@state'] = state, ['@fuel'] = fuel, ['@plate'] = plate, ['@citizenid'] = Player.PlayerData.citizenid})
        if state == 1 then
            exports.oxmysql:execute('UPDATE player_boats SET boathouse=@boathouse WHERE plate=@plate AND citizenid=@citizenid', {['@boathouse'] = boathouse, ['@plate'] = plate, ['@citizenid'] = Player.PlayerData.citizenid})
        end
    end
end)
