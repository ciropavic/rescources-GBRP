...
local volumes = {
	...
	['megaphone'] = tonumber(GetConvar('voice_defaultVolume', '0.3')) + 0.0,
}
...
local megaphoneEffectId = CreateAudioSubmix('megaphone')
...
isMegaphoneInUse = false

RegisterNetEvent("megaphone:toggle")
AddEventHandler("megaphone:toggle", function(pId, pEnable)
	toggleVoice(pId, pEnable, "megaphone")
	print(1)
	isUsingMegaphone = pEnable
	if isUsingMegaphone then
		local voiceModeData = Cfg.voiceModes[mode]
		MumbleSetAudioInputDistance(voiceModeData[1] + 10.0)
	else
		local voiceModeData = Cfg.voiceModes[mode]
...
function toggleVoice(plySource, enabled, moduleType)
	...
	print(enabled,moduleType)
	if enabled then
		MumbleSetVolumeOverrideByServerId(plySource, enabled and volumes[moduleType])
		print(GetConvarInt('voice_enableSubmix', 0))
...
RegisterCommand('+cycleproximity', function()
	...
	if isUsingMegaphone then
		voiceModeData[1]  = voiceModeData[1] + 10.0
	end
	MumbleSetAudioInputDistance(voiceModeData[1] + 0.0)
	mode = voiceMode
