        ...
        if receiptName ~= false or receiptName ~= nil then 
            ...
            if userOnline ~= false then
                ...
                -- User is not online so we need to manually adjust thier bank balance.
                    exports.oxmysql.scalar("SELECT `amount` FROM `bank_accounts` WHERE `account_number` = @an AND `sort_code` = @sc AND `character_id` = @cid", {
                        ['@an'] = data.account,
                        ['@sc'] = data.sortcode,
                        ['@cid'] = cid
                        ...
                        if currentBalance ~= nil then
                            ...
                            if newBalance ~= currentBalance then
                                exports.oxmysql:execute("UPDATE `bank_accounts` SET `amount` = @newBalance WHERE `account_number` = @an AND `sort_code` = @sc AND `character_id` = @cid", {
                                    ['@an'] = data.account,
                                    ['@sc'] = data.sortcode,
                                    ['@cid'] = cid,
