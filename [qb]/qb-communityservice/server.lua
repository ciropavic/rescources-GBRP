...
AddEventHandler('qb-communityservice:server:checkIfSentenced', function()
    ...
    local result = exports.oxmysql:executeSync('SELECT * FROM communityservice WHERE citizenid = ?', {citizenid})
...
AddEventHandler('qb-communityservice:server:completeService', function()
    local src = source
    local citizenid = QBCore.Functions.GetPlayer(src).PlayerData.citizenid
    exports.oxmysql:execute('UPDATE communityservice SET actions_remaining = actions_remaining - 1 WHERE citizenid = ?', {citizenid})
end)
...
AddEventHandler('qb-communityservice:server:extendService', function()
    local src = source
    local citizenid = QBCore.Functions.GetPlayer(src).PlayerData.citizenid
    exports.oxmysql:execute('UPDATE communityservice SET actions_remaining = actions_remaining + @extension_value WHERE citizenid = ?', {citizenid, Config.ServiceExtensionOnEscape})
end)
...
function Release(target)
    ...
    exports.oxmysql:execute('DELETE FROM communityservice WHERE citizenid = ?', {citizenid})
