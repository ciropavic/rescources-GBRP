...
AddEventHandler("qb-admin:server:kick", function(player, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions["kick"]) then
        ...
        DropPlayer(player.id, "You have been kicked from the server:\n" .. reason .. "\n\n🔸 Join the discord server for more information: https://discord.gg/sZtezNc4pQ")
    end
end)
...
AddEventHandler("qb-admin:server:ban", function(player, time, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions["ban"]) then
        ...
        if banTime >= 2147483647 then
            DropPlayer(player.id, "You have been banned:\n" .. reason .. "\n\nYour ban is permanent.\n🔸 Join the discord for more information: https://discord.gg/sZtezNc4pQ")
        else
...
QBCore.Commands.Add("blips", "Show blips for players (God Only)", {}, false, function(source, args) 
    TriggerClientEvent('qb-admin:client:toggleBlips', source)   
end, "god")                                                   

QBCore.Commands.Add("names", "Show player name overhead (God Only)", {}, false, function(source, args)   
    TriggerClientEvent('qb-admin:client:toggleNames', source)                                               
end, "god")

QBCore.Commands.Add("coords", "Enable coord display for development stuff (God Only)", {}, false, function(source, args)
    TriggerClientEvent('qb-admin:client:ToggleCoords', source)
end, "god")

QBCore.Commands.Add("admincar", "Save Vehicle To Your Garage (God Only)", {}, false, function(source, args)
...
QBCore.Commands.Add("checkwarns", "Check Player Warnings (Admin Only)", {{name="ID", help="Player"}, {name="Warning", help="Number of warning, (1, 2 or 3 etc..)"}}, false, function(source, args)
    if args[2] == nil then
        local targetPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
        local result = exports.oxmysql:executeSync('SELECT * FROM player_warns WHERE targetIdentifier=@targetIdentifier', {['@targetIdentifier'] = targetPlayer.PlayerData.license})
        TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", targetPlayer.PlayerData.name.." has "..tablelength(result).." warnings!")
    else
        local targetPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
        local warnings = exports.oxmysql:executeSync('SELECT * FROM player_warns WHERE targetIdentifier=@targetIdentifier', {['@targetIdentifier'] = targetPlayer.PlayerData.license})
...
QBCore.Commands.Add("delwarn", "Delete Players Warnings (God Only)", {{name="ID", help="Player"}, {name="Warning", help="Number of warning, (1, 2 or 3 etc..)"}}, true, function(source, args)
    local targetPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    local warnings = exports.oxmysql:executeSync('SELECT * FROM player_warns WHERE targetIdentifier=@targetIdentifier', {['@targetIdentifier'] = targetPlayer.PlayerData.license})
    local selectedWarning = tonumber(args[2])
    if warnings[selectedWarning] ~= nil then
...
QBCore.Commands.Add("setmodel", "Change Ped Model (God Only)", {{name="model", help="Name of the model"}, {name="id", help="Id of the Player (empty for yourself)"}}, false, function(source, args)
    local model = args[1]
...
QBCore.Commands.Add("setspeed", "Set Player Foot Speed (God Only)", {}, false, function(source, args)
    local speed = args[1]
    if speed ~= nil then
...
QBCore.Commands.Add("setammo", "Set Your Ammo Amount (God Only)", {{name="amount", help="Amount of bullets, for example: 20"}, {name="weapon", help="Name of the weapen, for example: WEAPON_VINTAGEPISTOL"}}, false, function(source, args)
    local src = source
    local weapon = args[2]
