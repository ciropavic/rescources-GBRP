...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
  PlayerJob = QBCore.Functions.GetPlayerData().gang
  PlayerJobPolice = QBCore.Functions.GetPlayerData().job
end)
...
Update = function()
  ...
  while true do
    ...
    if not dead then
      ...
      if lastZone then
        if PlayerJob and PlayerJob.name and GangLookup[PlayerJob.name] then
          TriggerServerEvent('Territories:LeaveZone',lastZone,PlayerJob.name)
          lastZone = false
        end  
      end 
    end
...
CheckLocation = function(closest)
  ...
  if closest then
    if plyHp > 100 then 
      if not lastZone or lastZone ~= closest then
        ...
        if not StopInfluence then
          ...
          lastZone = closest
          if PlayerJob and PlayerJob.name and GangLookup[PlayerJob.name] then
            TriggerServerEvent('Territories:EnterZone',closest,PlayerJob.name)
    ...
    if area.openzone or area.control == PlayerJob.name  then
      ...
      if ((Config.SlingByHotkey and GetKeyUp(Config.SlingDrugsControl)) or SlingNextFrame) and not CurSlinging then
        ...
        if closestDist and closestDist < Config.DrugSellDist then
...
SlingDrugs = function(area,key)
  ...
  local plyInventory = QBCore.Functions.GetPlayerData().items
  local hasSellable = 0
  for k,v in pairs(plyInventory) do
    if sellableLookup[v.name] and v.amount and v.amount > Config.MinSellAmount then
      playerSellables[v.name] = v.amount
    ...
    if not curSelling then
      ...
      if not soldPeds[curSelling] then
        ...
        for k,v in pairs(inventory) do
          if sellableLookup[v.name] and v.amount > Config.MinSellAmount then
            sellItem = v
          end
        end
        ...
        if sellItem then
          ...
          local amountToSell = math.random(math.min(Config.MinSellAmount,math.max(Config.MinSellAmount,sellItem.amount)))
...
SceneHandler = function(action)
  ...
  if hasItem  then
   ...
   -- progBar:taskBar(action.time,action.progText)
    QBCore.Functions.Progressbar("sell_pawn_items", action.progText, action.time, false, true, {}, {}, {}, {}, function() -- Done
...
UpdateBlips = function()
  for k,v in pairs(Territories) do    
    if Config.DrugProcessBlip then
      if not v.blip and PlayerData and (PlayerData and PlayerData.name and PlayerData.name == v.control or PlayerData.job2 and PlayerData.job2.name and PlayerData.job2.name == v.control) then
      ...
      elseif v.blip then
        ...
        inControl = (PlayerData and PlayerData and PlayerData.name and PlayerData.name == v.control)
...
PlayerInGang = function()
  if not PlayerData or (not PlayerData.job) or (not PlayerData.name) then return false; end
  if GangLookup[PlayerData.name] then 
    return true 
  end
end
...
SetJob = function(job)
  ...
  if lastZone then
    TriggerServerEvent('Territories:LeaveZone',lastZone,PlayerData.name)
    lastZone = false
  end
end
...
RegisterCommand('waypoint', function(source,args)
  ...
  if DoesBlipExist(WaypointHandle) then
    ...
    local b = AddBlipForCoord(waypointCoords.x,waypointCoords.y,waypointCoords.z)
    SetBlipSprite(b,51)
    SetBlipColour(b,1)
    SetBlipScale(b,1.0)
    --local blip = exports["meta_libs"]:AddBlip(waypointCoords.x,waypointCoords.y,waypointCoords.z,51,1,"Testing",1.0,2,false,true)
    SetBlipRoute(b,true)
  end
end)
