...
QBCore.Functions.CreateUseableItem("pd_heavyarmor", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)
    local Job = Player.PlayerData.job.name
    if Job == "police" then
        TriggerClientEvent('consumables:client:UseHeavyArmor', source, item)
    else
...
QBCore.Functions.CreateUseableItem("starter_gift", function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("cpl_starter:client:givestarteritem", source)
end)

RegisterServerEvent('cpl_starter:server:giveitem')
AddEventHandler('cpl_starter:server:giveitem', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.AddItem(item, 1)
end)

RegisterServerEvent('cpl_starter:server:removebox')
AddEventHandler('cpl_starter:server:removebox', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
