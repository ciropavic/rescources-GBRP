...
RegisterNetEvent('cpl_starter:client:givestarteritem')
AddEventHandler('cpl_starter:client:givestarteritem', function()
    local item_array = { 'tosti', 'joint', 'pistol_ammo', 'advancedrepairkit', 'advancedlockpick', 'lockpick', 'armor', 'sandwich', 'firework1', 'firework4', 'harness', 'bandage', 'painkillers', 'fitbit' }
    ...
    for i,v in ipairs(item_array ) do
        ...
        if itemchance == 1 then
            TriggerServerEvent('cpl_starter:server:giveitem', v) --Give the item to the user using server event
        end
    end
    
    TriggerServerEvent('cpl_starter:server:removebox') --Remove the item once used
end)
