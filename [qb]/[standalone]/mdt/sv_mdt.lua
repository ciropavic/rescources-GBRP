...
AddEventHandler("mdt:hotKeyOpen", function()
    ...
    if xPlayer.PlayerData.job.name == 'police' then
    	exports.oxmysql:execute("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		for r = 1, #reports do
    			reports[r].charges = json.decode(reports[r].charges)
    		end
    		exports.oxmysql:execute("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			for w = 1, #warrants do
...
AddEventHandler("mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	exports.oxmysql:execute('SELECT * FROM fine_types', {}, function(fines)
		for j = 1, #fines do
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `players` WHERE `charinfo` LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
...
AddEventHandler("mdt:getOffenderDetails", function(offender)
    ...
    exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {offender.id}, function(result)
        ...
        exports.oxmysql:execute('SELECT * FROM `user_convictions` WHERE `char_id` = ?', {offender.id}, function(convictions)
            ...
            exports.oxmysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {offender.id}, function(warrants)

                if warrants[1] then
                    offender.haswarrant = true
                end

				exports.oxmysql:execute('SELECT * FROM `player_vehicles` WHERE `citizenid` = ?', {offender.id}, function(vehicles)
					for i = 1, #vehicles do
        ...
        exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {offender.id}, function(result)
            ...
            exports.oxmysql:execute('SELECT * FROM `user_convictions` WHERE `char_id` = ?', {offender.id}, function(convictions) 
                ...
                exports.oxmysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {offender.id}, function(warrants)
                    ...
                    exports.oxmysql:execute('SELECT * FROM `player_vehicles` WHERE `citizenid` = ?', {offender.citizenid}, function(vehicles)
                        for i = 1, #vehicles do
                            vehicles[i].model = vehicles[i].vehicle
                            if vehicles[i].mods then
                                local vehmods = json.decode(vehicles[i].mods)
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {id}, function(result)
		if result[1] then
...
AddEventHandler("mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `mdt_reports` WHERE `id` LIKE :test OR LOWER(`title`) LIKE :test OR LOWER(`name`) LIKE :test OR LOWER(`author`) LIKE :test or LOWER(`charges`) LIKE :test", {
...
AddEventHandler("mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	exports.oxmysql:execute("SELECT * FROM `player_vehicles` WHERE LOWER(`plate`) LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
		for index, data in ipairs(result) do
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	local usource = source
    exports.oxmysql:execute("SELECT * FROM `players` WHERE `citizenid` = ?", {vehicle.citizenid}, function(result)
        ...
        exports.oxmysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = ?', {vehicle.plate}, function(data)
            ...
            exports.oxmysql:execute('SELECT * FROM `mdt_warrants` WHERE `char_id` = ?', {vehicle.owner_id}, function(warrants)

                if warrants[1] then
                    vehicle.haswarrant = true
                end

                exports.oxmysql:execute('SELECT `bail` FROM user_mdt WHERE `char_id` = ?', {vehicle.owner_id}, function(bail)
...
AddEventHandler("mdt:getWarrants", function()
	local usource = source
	exports.oxmysql:execute("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		for i = 1, #warrants do
...
AddEventHandler("mdt:getReportDetailsById", function(query, _source)
	...
	local usource = source
	exports.oxmysql:execute("SELECT * FROM `mdt_reports` WHERE `id` = ?", {query}, function(result)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	local usource = source
	exports.oxmysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = ?', {data.plate}, function(result)
		if result[1] then
