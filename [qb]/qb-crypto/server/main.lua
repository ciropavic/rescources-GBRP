    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                exports.oxmysql:execute('INSERT INTO crypto (worth, history) VALUES (@worth, @history) ON DUPLICATE KEY UPDATE worth = @worth, history = @history', {
                    ['@worth'] = NewWorth,
...
AddEventHandler('qb-crypto:server:FetchWorth', function()
    for name,_ in pairs(Crypto.Worth) do
        local result = exports.oxmysql:executeSync('SELECT * FROM crypto WHERE crypto=@crypto', {['@crypto'] = name})
...
QBCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        local result = exports.oxmysql:executeSync('SELECT * FROM `players` WHERE `metadata` LIKE @query', {['@query'] = query})
        if result[1] ~= nil then
            local CryptoData = {
                History = Crypto.History["qbit"],
                Worth = Crypto.Worth["qbit"],
...
RefreshCrypto = function()
    local result = exports.oxmysql:executeSync('SELECT * FROM crypto WHERE crypto = @crypto', {['@crypto'] = coin})
