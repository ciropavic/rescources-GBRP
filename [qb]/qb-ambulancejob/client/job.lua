...
RegisterNetEvent('HospitalCheckIn')
AddEventHandler('HospitalCheckIn', function()
    if QBCore ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
            ...
            QBCore.Functions.Notify("you are dead", "error", 5000) -- [text] = message, [type] = primary | error | success, [length] = time till fadeout.
        end
    end
end)

RegisterNetEvent('toggleEMSduty')
AddEventHandler('toggleEMSduty', function()
    onDuty = not onDuty
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("police:server:UpdateBlips")
end)

RegisterNetEvent('ems:armoury')
AddEventHandler('ems:armoury', function()
    if onDuty then
...
RegisterNetEvent('qb-ambulancejob:client:toRoof', function()
    local currenthosp = 1
    local ped = PlayerPedId()
    for k, v in pairs(Config.Locations["main"]) do
        if #(GetEntityCoords(ped) - vector3(v.x, v.y, v.z)) < 1.5 then
            currenthosp = k
        end
    end
    DoScreenFadeOut(500)
    ...
    local coords = Config.Locations["roof"][currenthosp]
    SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
    SetEntityHeading(ped, coords.w)
...
RegisterNetEvent('qb-ambulancejob:client:toground', function()
    local currenthosp = 1
    local ped = PlayerPedId()
    for k, v in pairs(Config.Locations["main"]) do
        if #(GetEntityCoords(ped) - vector3(v.x, v.y, v.z)) < 1.5 then
    ...
    local coords = Config.Locations["ground"][currenthosp]
    SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
    SetEntityHeading(ped, coords.w)
...
RegisterNetEvent('qb-ambulancejob:client:toMain', function()
    local currenthosp = 1
    local ped = PlayerPedId()
    for k, v in pairs(Config.Locations["roof"]) do
        if #(GetEntityCoords(ped) - vector3(v.x, v.y, v.z)) < 1.5 then
    ...
    local coords = Config.Locations["main"][currenthosp]
    SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
    SetEntityHeading(ped, coords.w)
...
RegisterNetEvent('qb-ambulancejob:client:toGarage', function()
    local currenthosp = 1
    local ped = PlayerPedId()
    for k, v in pairs(Config.Locations["main"]) do
        if #(GetEntityCoords(ped) - vector3(v.x, v.y, v.z)) < 1.5 then
    ...
    local coords = Config.Locations["garage"][currenthosp]
    SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
    SetEntityHeading(ped, coords.w)
...
RegisterNetEvent('ambulance:menu', function()
    TriggerEvent('nh-context:sendMenu', {
        ...
        },
        {
            id = 2,
            header = "To The Roof",
            txt = "Take the elevator to the Roof",
            params = {
                event = "qb-ambulancejob:client:toRoof",
                args = {
                }
            }
       },
      {
                    id = 3,
                    header = "To The Main Floor",
                    txt = "Take the elevator to the Main Floor",
                    params = {
                        event = "qb-ambulancejob:client:toMain",
                        args = {
                        }
                    }
        },
        {
            id = 4,
            header = "To The Ground Floor",
            txt = "Take the elevator to the Ground Floor",
            params = {
                event = "qb-ambulancejob:client:toground",
                args = {
                }
            }
          },
          {
            id = 5,
            header = "To The Garage",
            txt = "Take the elevator to the Garage",
            params = {
                event = "qb-ambulancejob:client:toGarage",
                args = {
                }
            }
          },
...
RegisterNetEvent('ems:personalstash')
