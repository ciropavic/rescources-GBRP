...
QBCore.Commands.Add("deletedealer", "Delete A Dealer (Admin Only)", {
    ...
    local dealerName = args[1]
    local result = exports.oxmysql:executeSync('SELECT * FROM dealers WHERE name=@name', {['@name'] = dealerName})
...
AddEventHandler('qb-drugs:server:CreateDealer', function(DealerData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = exports.oxmysql:executeSync('SELECT * FROM dealers WHERE name=@name', {['@name'] = DealerData.name})
    if result[1] ~= nil then
