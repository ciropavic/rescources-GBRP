...
QBCore.Functions.IsWhitelisted = function(source)
	...
	if (QBCore.Config.Server.whitelist) then
		local result = exports.oxmysql:executeSync('SELECT * FROM whitelist WHERE license = ?', { QBCore.Functions.GetIdentifier(source, 'license') })
		local data = result[1]
		if data ~= nil then
...
QBCore.Functions.IsPlayerBanned = function (source)
	local retval = false
	local message = ""
    local result = exports.oxmysql:executeSync('SELECT * FROM bans WHERE license = ?', { QBCore.Functions.GetIdentifier(source, 'license') })
