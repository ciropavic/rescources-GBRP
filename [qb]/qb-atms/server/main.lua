...
RegisterServerEvent('qb-atms:server:tryATM')
AddEventHandler('qb-atms:server:tryATM', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if visas ~= nil and masters ~= nil then
        for _, v in pairs(visas) do
            ...
            if xCH ~= nil then
                ...
                local player = exports.oxmysql:executeSync('SELECT charinfo FROM players WHERE citizenid=@citizenid', {['@citizenid'] = info.citizenid})
    ...
    if data ~= nil then 
        ...
        if dailyWith < 5000 then
            ...
            if xCH ~= nil then
                ...
                local player = exports.oxmysql:executeSync('SELECT * FROM players WHERE citizenid=@citizenid', {['@citizenid'] = cardHolder})
